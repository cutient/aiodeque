...
- Lower minimum Python version from 3.14 to 3.11

### Added

- `evict=True` keyword argument for `aappend()`, `aappendleft()`, `aextend()`, and `aextendleft()` — evicts the oldest element from the opposite end instead of blocking when full; returns the evicted element or `None`
